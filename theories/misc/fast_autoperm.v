...
Lemma map_SeqE (A B : Type) (f : A -> B) :
  (forall i : A, SeqAtom (f i) = map_Seq f (SeqAtom i)) *
...
Ltac seqReify xs :=
  lazymatch xs with
    | @nil ?A => constr:(SeqNil (seq A))
    | ?x :: ?xs =>
      let xs' := seqReify xs in constr:(SeqCat (SeqAtom [:: x]) xs')
    | rcons ?xs ?x =>
      let xs' := seqReify xs in constr:(SeqCat xs' (SeqAtom [:: x]))
    | ?xs ++ ?ys =>
...
Ltac reify_permeqs A eqs :=
  let reify_permeq xs ys :=
    let xs' := seqReify xs in
    let ys' := seqReify ys in
    constr: (perm_eq (denote_Seq id xs')
                     (denote_Seq id ys'))
  in
...
Ltac autoperm :=
  let rec rereify f e :=
    lazymatch e with
      | SeqAtom (f ?i) => constr: (SeqAtom i)
      | @SeqNil _ => constr: (@SeqNil rindex)
      | SeqCat ?el ?er =>
        let el' := rereify f el in
        let er' := rereify f er in
        constr: (SeqCat el' er')
      | SeqRev ?e' => let e'' := rereify f e' in constr: (SeqRev e'')
    end
  in
  let rec rereify_eqs f e :=
    lazymatch e with
      | context ctx [?peq = perm_eq (denote_Seq id ?el) (denote_Seq _ ?er)] =>
        let el' := rereify f el in
        let er' := rereify f er in
  ...
  in
  let do_autoperm A :=
    let eqs := fresh "eqs" in reify_permeqs A eqs;
    let f' := fresh "f" in pose f' := tt;
    myquate f' eqs (@SeqAtom (seq A)) (@nil A);
    lazymatch goal with
      | eqs := ?eqs' |- _ =>
        let eqs'' := rereify_eqs f' eqs' in
        clear eqs;
...
Proof.
autoperm.
(* Finished transaction in 30.886 secs (30.727u,0.059s) (successful) *)

by rewrite !eqs.

Time Defined.
(* Finished transaction in 0.629 secs (0.624u,0.003s) (successful) *)
