...
Ltac reify_permeqs A eqs :=
  let reify_permeq xs ys :=
    let xs' := seqReify xs in
    let ys' := seqReify ys in
    constr: (perm_eq (denote_Seq id xs') (denote_Seq id ys'))
  in
  let rec reify_rec eqs' :=
    let peq := fresh "peq" in
    lazymatch goal with
      | |- context [@perm_eq A ?xs ?ys] =>
        let peq' := reify_permeq xs ys in
        set peq := (perm_eq _ _); reify_rec (eqs' * (peq = peq'))%type
      | H : context [@perm_eq A ?xs ?ys] |- _ =>
        let peq' := reify_permeq xs ys in
        set peq := (perm_eq _ _) in H; reify_rec (eqs' * (peq = peq'))%type
      | x := context [@perm_eq A ?xs ?ys] |- _ =>
        let peq' := reify_permeq xs ys in
        set peq := (perm_eq _ _) in x; reify_rec (eqs' * (peq = peq'))%type
    end ||
...
Time
let rec autoperm_num_rew eqs f n i :=
  fold (f i) in eqs;
  try lazymatch n with ?n'.+1 =>
      autoperm_num_rew eqs f n' (rindex_L i);
      autoperm_num_rew eqs f n' (rindex_R i)
  end
in
let do_autoperm A :=
  let SeqEmbed' := fresh "SeqEmbed'" in pose SeqEmbed' := @SeqEmbed;
  let f' := fresh "f" in set f' := tt;
  let eqs := fresh "eqs" in
  let rec autoperm_pop fs f n :=
    lazymatch fs with
      | @nil _ =>
        let f := eval cbv beta in f in
        clear f'; set f' := f;
        unfold SeqEmbed' in eqs;
        autoperm_num_rew eqs f' n rindex_C
      | cons None ?fstail =>
        autoperm_pop fstail f n.+1
      | cons (Some (?fl, ?xs')) ?fstail =>
        autoperm_pop fstail
                     (fun i => match i with
                                 | rindex_L i' => fl i'
                                 | rindex_C => xs'
  ...
  in
  let rec autoperm_num fs :=
    lazymatch goal with
      | eqs := context [@SeqEmbed _ ?xs] |- _ =>
        fold (SeqEmbed' _ xs) in eqs;
        lazymatch goal with
          | eqs := context [@SeqEmbed _ ?ys] |- _ =>
            fold (SeqEmbed' _ ys) in eqs;
            autoperm_push fs (fun _ : rindex => xs) ys 0
          | _ => autoperm_pop fs (fun _ : rindex => xs) 1
        end
      | _ => autoperm_pop fs (fun _ : rindex => @nil A) 1
    end
  with autoperm_push fs f xs n :=
    lazymatch fs with
      | @nil _ => autoperm_push_f [:: Some (f, xs)] n
      | cons None ?fstail =>
        autoperm_push_f (Some (f, xs) :: fstail) n
      | cons (Some (?fl, ?xs')) ?fstail =>
        autoperm_push fstail
                      (fun i => match i with
                                  | rindex_L i' => fl i'
                                  | rindex_C => xs'
                                  | rindex_R i' => f i'
  ...
  with autoperm_push_f fs n :=
    lazymatch n with
      | ?n'.+1 => autoperm_push_f (None :: fs) n'
      | 0 => autoperm_num fs
    end
  in
  let rec rereify e :=
    lazymatch e with
      | SeqEmbed (f' ?i) => constr: (SeqEmbed i)
      | @SeqNil _ => constr: (@SeqNil rindex)
      | SeqCat ?el ?er =>
        let el' := rereify el in
        let er' := rereify er in
        constr: (SeqCat el' er')
      | SeqRev ?e' => let e'' := rereify e' in constr: (SeqRev e'')
    end
  in
  let rec rereify_eqs e :=
    lazymatch e with
      | context ctx [?peq = perm_eq (denote_Seq id ?el) (denote_Seq id ?er)] =>
        let el' := rereify el in
        let er' := rereify er in
        let e' := context ctx [peq = perm_eq (denote_Seq f' el')
                                             (denote_Seq f' er')] in
        rereify_eqs e'
      | _ => constr: (e)
    end
  in
  reify_permeqs A eqs;
  autoperm_num (@nil (option ((rindex -> seq A) * seq A)));
  lazymatch goal with
    | [eqs := ?eqs' |- _] =>
      let eqs'' := rereify_eqs eqs' in
      clear eqs;
      pose eqs := eqs''
  end;
  let Heqs := fresh "Heqs" in
  (have Heqs: eqs by repeat
     split;
     lazymatch goal with
       |- ?peq = _ => rewrite /peq -?cats1 ?catrevE; reflexivity
     end);
  subst eqs; rewrite ?denote_map_Seq !perm_eq_simpl in Heqs;
  repeat
    (let pe := fresh "pe" in set pe := perm_elim _ _ _ in Heqs;
     native_compute in pe; subst pe);
  cbv iota beta in Heqs
in
lazymatch goal with
  | [|- context [@perm_eq ?A ?xs ?ys]] => do_autoperm A
  | H : context [@perm_eq ?A ?xs ?ys] |- _ => do_autoperm A
  | x := context [@perm_eq ?A ?xs ?ys] |- _ => do_autoperm A
end.
(* Finished transaction in 35.253 secs (35.106u,0.039s) (successful) *)

by rewrite Heqs.

Time Defined.
(* Finished transaction in 0.565 secs (0.565u,0.s) (successful) *)
