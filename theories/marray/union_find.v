...
Definition cdatum x := if g x is inr a then a else Ridx.
Definition repr x := if g x is inr _ then true else false.
...
apply: connect_trans _ (IH _ H0 H1); rewrite /succ.
...
Lemma reprP x : reflect (g x = inr (cdatum x)) (repr x).
Proof. by rewrite /repr /cdatum; case: (g x) => ?; constructor. Qed.

Lemma reprPn x : reflect (g x = inl (succ x)) (~~ repr x).
Proof. by rewrite /repr /succ; case: (g x) => ?; constructor. Qed.

Lemma repr_succ x : repr x -> repr (succ x).
Proof. by rewrite /repr /succ; case_eq (g x) => // ? ->. Qed.
...
Proof.
move=> H H0 y.
rewrite [RHS]unfold_in -/(fingraph.connect _ _ _) -/(connect _ _).
elim/path_ind: x xs / H H0 => /= [x H | x xs H Hxs IH H0];
  last by rewrite inE IH // eq_sym -connectE.
rewrite !inE; apply/eqP/idP => [-> |]; first exact: connect0.
by case/connectP => -[] //= x' xs /andP [/eqP]; move/reprP: H => ->.
Qed.
...
Proof. by rewrite /succ; move/reprP: Hx => ->. Qed.
...
Proof.
apply/idP/idP; first by rewrite /succ => /reprP ->.
by rewrite -find_invariance => /eqP ->.
Qed.
...
rewrite IH ?lacycle_succ // path_notin //= Hxs.
by move/reprPn: H => ->; rewrite eqxx.
Qed.
...
Proof.
by move=> Hxy; rewrite /lacycle find_subst_separated // /repr ffunE;
...
elim/path_ind: y ys / Hys => [y | y ys H Hys] /=;
  first by apply/repr_iter/reprP; rewrite /cdatum ffunE eqxx.
rewrite -(lacycle_succ _ y) succ_subst; case: eqP H Hys => //.
rewrite /repr /succ; case: (g y) => // y' H H0 H1 _; subst y.
by apply: lacycle_repr; rewrite /repr ffunE eqxx.
Qed.
...
have {H0 Hxs xs'} Hxs: path (ffun_set (last x xs) (inr a) g) x xs by
  ...
  move/reprPn: H0 H1; rewrite {}IH // andbT ffunE /succ;
...
- rewrite -/(path _) => Hxs Hxs' _ -> {y xs'}.
  ...
  by apply/connect_trans/connect_succL; rewrite succ_subst;
...
elim/path_ind: x xs / H1 H H0 => /= [x H H0 | x xs H H0 H1 H2 H3];
  rewrite ffunE ?eqxx //.
have/(path_notin H2): path g x (succ g x :: xs) by rewrite /= (reprPn H) eqxx.
case: (x =P _) => [{1}-> | _ _]; first by rewrite mem_last.
rewrite (reprPn H) eqxx H1 ?lacycle_succ //.
apply/contra/connect_trans/connect1: H3.
Qed.

Lemma repr_compress x y : repr g y = repr (compress x) y.
Proof.
rewrite /repr /compress !ffunE /(succ _ _).
...
case: (altP andP) => [[] H2 |]; last by move/reprPn/eqP: H.
...
Lemma compress_repr x y : repr g x -> compress y x = inr (cdatum g x).
Proof.
rewrite /repr /cdatum ffunE /find; case: andP;
...
by case: xs => [[] H0 _ | x' xs [] H0] _ _ /=; last apply: connect_trans;
...
Proof. by move=> ?; rewrite /lacycle /repr find_compress ?compress_repr. Qed.
...
Proof. move=> ?; exact: lacycle_compress. Qed.
...
elim/path_ind: x xs / H (path_uniq (Hg _) H) => [x /= _ H | x xs];
  first by rewrite !(run_AStateE, reprP H); congr (_, _);
...
move/reprP: H6 => H6; rewrite !inE; do !case: eqP => //=; congruence.
Qed.

Lemma run_mfind x :
  run_AState (mfind x) g = (find g x, cdatum g (find g x), compress g x).
Proof.
...
Definition union x y :=
  ...
  let v  := Rop (cdatum g x') (cdatum g y') in
  let g' := compress (compress g x) y in
  if findeq g x y then g' else
    if cmp (cdatum g x') (cdatum g y')
...
by rewrite find_compress // /(cdatum (compress _ _)) compress_repr;
