...
Variant up_search_spec (i j : 'I_#|I|.+1) (arr : {ffun I -> A}) :
  ...
  | UpSearchSpec (k : 'I_#|I|.+1) :
    ...
    up_search_spec i j arr (k, arr).

Lemma run_up_search (i j : 'I_#|I|.+1) (arr : {ffun I -> A}) :
  i <= j -> up_search_spec i j arr (run_AState (up_search i j) arr).
Proof.
...
CoInductive down_search_spec (i j : 'I_#|I|.+1) (arr : {ffun I -> A}) :
  ...
  | DownSearchSpec (k : 'I_#|I|.+1):
    ...
    down_search_spec i j arr (k, arr).

Lemma run_down_search (i j : 'I_#|I|.+1) (arr : {ffun I -> A}) :
  j <= i -> down_search_spec i j arr (run_AState (down_search i j) arr).
Proof.
...
CoInductive partition_spec
  ...
  partition_spec pivot i j arr (k, arr').
...
elim: n i j arr => [| n IH] i j arr;
  rewrite !(run_AStateE, addn0, addnS) => H /=;
  first by
...
rewrite !(run_AStateE, run_SWAP) => H1 Hpart1 Hpart2 Hpart3.
set arr' := [ffun _ => _].
have {arr'} ->:
...
CoInductive quicksort_spec (arr : {ffun I -> A}) :
  ...
  QuicksortSpec (p : {perm I}) :
    ...
    quicksort_spec arr (tt, [ffun ix => arr (p ix)]).
