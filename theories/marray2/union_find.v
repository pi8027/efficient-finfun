...
rewrite !inE; apply/idP/eqP => [| ->]; last by apply connect0.
...
case/connectP => xs Hxs ->;
  elim/path_ind: x xs / Hxs => [| x xs H]; first by apply connect0.
...
Fixpoint mfind_rec n x : AState {ffun T -> T + R} {ffun T -> T + R} (T * R) :=
  if n is n'.+1
  then x' <- astate_get x;
       match x' with
         | inl x'' =>
...
elim/path_ind: x xs / H (path_uniq (Hg _) H) => [x /= _ H | x xs];
  first by rewrite !(run_AStateE, eqP H);
           congr (_, _); apply/ffunP => z; rewrite ffunE inE andbN.
rewrite reprE // /(succ g x); case_eq (g x) => x';
  rewrite ?eqxx //= !run_AStateE inE negb_or -andbA
...
Lemma run_mfind x :
  run_AState (mfind x) g =
  (find g x, classval g (find g x), compress g x).
Proof.
...
Definition munion x y : AState {ffun T -> T + R} {ffun T -> T + R} unit :=
  x' <- mfind x;
  y' <- mfind y;
...
Extraction "../../ocaml/wuf_o0.ml" runt_AState WUF.

Set Extraction Flag 8175.

Extract Type Arity AState 1.

Extraction "../../ocaml/wuf.ml" runt_AState WUF.
*)
