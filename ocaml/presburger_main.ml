...
let sat vars f =
    let (time, res) = Utils.with_timer_median 5 (fun _ -> presburger_sat vars f) in
    (if res then "SAT" else "UNSAT") ^ " (time: " ^ Utils.string_of_float time ^ "s)"
;;
...
let dec vars f =
    let (time, res) = Utils.with_timer_median 5 (fun _ -> presburger_st_dec f) in
    (if res then "TRUE" else "FALSE") ^ " (time: " ^ Utils.string_of_float time ^ "s)"
;;
